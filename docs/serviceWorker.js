...
self.addEventListener('install', event => {
    ...
    event.waitUntil(
        ...
        caches.open(staticCache).then(cache => {
            return cache.addAll(staticFilestoCach)
        }   
    ))
})
...
function tryToLoadFromNetworkAndUpdateCacheIfSuccessful (request, timeout = 10000) {
    ...
    return new Promise((resolve, reject) => {
        ...
        fetch(request).then(response => {
            ...
            if (staticFilestoCach.includes(request.url)){
...
self.addEventListener('fetch', event => {
    ...
    event.respondWith(
        tryToLoadFromNetworkAndUpdateCacheIfSuccessful(event.request).catch(() => {
